...
async function main() {
  ...
  const ethAddress = rightAlignBuffer(
    Buffer.from(
      hexStringToUint8Array("0xa550C6011DfBA4925abEb0B48104062682870BB8")
    )
  );
  const addressKey = await deriveEthAddressKey(
    HELLO_WORLD_PID,
  ...
  const buf = Buffer.alloc(8);
  buf.writeBigUint64LE(BigInt(100000000), 0); // 跨链转的金额，需要至少一位精度，也就是说转0.001不行，转0.1可以，好像是跨链桥的限制
  const addressBuf = Buffer.from(
    hexStringToUint8Array("0x049B426457B5A75e0e25F0b692dF581a06035647")
  ); // 要跨链到的以太坊地址
  const encodedParams = Buffer.concat([paras, buf, addressBuf]);
  console.log(encodedParams);

  const encodeMeta = borsh.serialize(AccountMeta, [
  ...
  const RawData = {
    ...
    accounts: [
      ...
      {
        key: new PublicKey(
          "HD4ktk6LUewd5vMePdQF6ZtvKi3mC41AD3ZM3qJW8N8e"
        ).toBuffer(), //会现将sol转给这个地址，由他帮你跨链过去
        isWritable: true,
        isSigner: false,
      },
    ],
  ...
  const uniProxy_factory = await ethers.getContractFactory("UniProxy");
  const UniProxy = await uniProxy_factory.attach(
    "0x438aCC4fB994D97A052d225f0Ca3BF720a3552A9"
  );
  const receipt = await UniProxy.sendMessage(RawDataEncoded);
  console.log(receipt.hash);
  ...
  const userAddress = coder.encode(
    ...
    [
      Buffer.from(
        new PublicKey("6v9YRMJbiXSjwco3evS2XdNuqPbwzKf3ykmn5iQJ4UyF").toBytes()
      ),
    ]
  ); //自己的solana地址
  // approve wsol,这个操作不是每次都需要
  const byte32WsolContract = coder.encode(
    ["bytes32"],
    [
      ethers.zeroPadValue(
        Buffer.from(hexStringToUint8Array(WSOL_CONTRACT_ADDRESS)),
        32
      ),
    ]
  );
  ...
  let approveParas = approveIface.encodeFunctionData("approve", [
    TOKEN_BRIDGE_RELAYER_CONTRACT,
    BigInt(
      "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
    ),
  ]);
  let approvePayloadPart = coder.encode(
    ["bytes32", "uint256", "bytes"],
    [byte32WsolContract, 0, approveParas]
  );
  const approvePayload = coder.encode(
    ["bytes32", "bytes"],
    [userAddress, approvePayloadPart]
  );
