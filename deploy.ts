...
async function main() {
  ...
  const NFT_VERIFICATION_CONTRACT =
    "6QBQwCw7gYQGb4aTW5Hxexcms24AnJRyU9pBCKhDLNSq";
  ...
  const reserved = 0;
  // solana ---> sepolia
  const solanaPayloadHead = Buffer.concat([
  ...
  // sepolia ---> solana
  const sepoliaPayloadHead = Buffer.concat([
    Buffer.from([0xfe, 0x01, 0x00, 0x00]),
    sepoliaChainIdBuffer,
    solanaChainIdBuffer,
    Buffer.alloc(reserved),
  ...
  const userSolanaAddress = ethers.zeroPadValue(
    new PublicKey(USER_SOLANA_ADDRESS).toBytes(),
    32
  );
  ...
  const userAddress = coder.encode(
    ["bytes32"],
    [Buffer.from(new PublicKey(USER_SOLANA_ADDRESS).toBytes())]
  );
  ...
  const contract_address = coder.encode(
    ...
    [
      ethers.zeroPadValue(
        Buffer.from(hexStringToUint8Array(NFT_PROOF_CONTRACT)),
        32
      ),
    ]
  ...
  let iface = new ethers.Interface(ABI);
  let paras = iface.encodeFunctionData("sendProof", [
    nftContractToken,
    payloadSend,
  ]);
  ...
  const payload = coder.encode(
    ["bytes8", "bytes32", "bytes"],
    [solanaPayloadHead, userAddress, payload_part]
  );
  console.log(payload);
