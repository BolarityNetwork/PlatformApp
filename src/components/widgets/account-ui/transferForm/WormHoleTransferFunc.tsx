...
function WormHoleTransferFunc() {
  ...
  const solanaTransferSplBolarityToSolana = async ({
    toPubkey,
    balance,
  }: {
    toPubkey: PublicKey;
    ...
    try {
      console.log("CLAIM_TOKEN_CONTRACT---", CLAIM_TOKEN_CONTRACT);
      // 获取BOLARITY代币地址
      const tokenMintPublicKey = new PublicKey(CLAIM_TOKEN_CONTRACT);
      const fromPubkey = new PublicKey(solAddress);
      // 获取发送方和接收方的关联账户地址
      const senderTokenAccount = await getAssociatedTokenAddress(
        tokenMintPublicKey,
      ...
      tx.add(
        createTransferInstruction(
          ...
          recipientTokenAccount,
          fromPubkey,
          parseUnits(balance.toString(), 9) // BOLARITY精度为9
        )
      );
      ...
      console.error("BOLARITY transfer failed: ", error);
      toast.error("BOLARITY transfer failed: " + error);
    }
  };
  ...
  return {
    ...
    solanaTransferSplBolarityToSolana,
  };
}
